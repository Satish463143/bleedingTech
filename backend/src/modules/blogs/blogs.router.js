...
router.route('/')
    .post(loginCheck, hasPermission("admin"), setPath("blogs"), uplaodFile(FileFilterType.IMAGE).fields([
        { name: 'thumbnail', maxCount: 1 },
        { name: 'heroImage', maxCount: 1 },
        { name: 'authorAvatar', maxCount: 1 }
    ]), persistAllToS3, bodyvalidator(blogsDTO), blogsController.createBlogs)
    .get(blogsController.index)

router.route('/:id')
    .get(blogsController.showById)
    .put(loginCheck, hasPermission("admin"), setPath("blogs"), uplaodFile(FileFilterType.IMAGE).fields([
        { name: 'thumbnail', maxCount: 1 },
        { name: 'heroImage', maxCount: 1 },
        { name: 'authorAvatar', maxCount: 1 }
    ]), persistAllToS3, bodyvalidator(blogsDTO), blogsController.updateBlogs)
