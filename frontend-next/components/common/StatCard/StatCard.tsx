...
function useCounter({ end, duration = 2000, start = 0, inView = false }: UseCounterParams) {
...
type StatCardProps = {
  ...
  suffix?: string;
  label: string;
  color: string; // e.g. "#ff00aa" or "hsl(...)"
  index: number;
  inView: boolean;
};
...
export default function StatCard({
  ...
  const count = useCounter({ end: value, duration: 2000, start: 0, inView });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mb-3">
          <motion.span className="text-4xl lg:text-5xl font-black" style={{ color }}>
            {count}
            {suffix}
          </motion.span>
        </div>
        ...
        <p className="text-sm lg:text-base font-medium" style={{ color: "hsl(var(--muted-foreground))" }}>
          {label}
        </p>
